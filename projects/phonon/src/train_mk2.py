            ...
            if save:
                torch.save({"state": model.state_dict(), "history": history,
                            ...
                            "loss_cfg": {"dist": crit.dist, "alpha": crit.alpha,
                                         "beta": crit.beta}}, ckpt_path)
            if not quiet:
                print(f"  [ckpt] epoch {epoch+1:3d}  val MSE={mse:.4f}  "
                      f"frac@10%={frac:.3f}  EMD={emd:.2f}cm^-1  "
                      f"elapsed={time.strftime('%H:%M:%S', time.gmtime(wall))}")
        sched.step()

    val_mse, _, val_frac, val_emd = eval_loader(model, dl_va, crit, freq, device)
    return model, history, val_frac, val_emd
